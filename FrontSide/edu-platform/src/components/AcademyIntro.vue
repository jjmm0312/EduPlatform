<template>
  <div id="main-text">
    <h1 id="main-title">{{AcademyName}}</h1>
    <hr id="main-line" />
    <!-- <div id="title-box">
      <span id="notice-title">{{AcademyName}}</span>
    </div> -->

    <!-- <hr /> -->
    <p id="notice-content">{{Introduction}}</p>
...
export default {
  ...
  created() {
      var vm = this;
    axios
      .get("http://" + IP.IP + ":8080/manage/academy-info", {
        timeout: 10000, // 1초 이내에 응답이 없으면 에러 처리
      })
      .then((res) => {
        console.log(res.data);

        vm.AcademyName = res.data.name;
        vm.Introduction = res.data.intro;
